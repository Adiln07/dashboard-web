...
const ModalEditProductUser = () => {
  const productId = useUsersProductStore((state) => state.productId);
  const productById = useUsersProductStore((state) => state.productById);
  const isEditProduct = useUsersProductStore((state) => state.isEditProduct);
  const editProduct = useUsersProductStore((state) => state.editProduct);
  const fetchGetProductById = useUsersProductStore(
    (state) => state.fetchGetProductById,
  );
  const isEditClosed = useUsersProductStore(
    (state) => state.isEditProductClosed,
  );
  ...
  useEffect(() => {
    if (productId && isEditProduct) {
      fetchGetProductById(productId);
    }
  }, [productId]);
        ...
        <form
          ...
          onSubmit={async (e) => {
            ...
            if (fotoFile) {
              ...
              editProduct({ ...editData, image: imageUrl }, productId);
            } else {
              editProduct(editData, productId);
            }
          }}
        >
...
export default ModalEditProductUser;
